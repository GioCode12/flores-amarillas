...
function distribuirRamilleteSuave(count, centroX, centroY, spreadX, spreadY, baseX, baseY) {
    ...
    capas.forEach((capa, idx) => {
        ...
        for (const p of pts) puntos.push({
            ...p,
            z: capa.z,
            radioMul: capa.radioMul
        });
    });
...
function getConfigMedidasFijas() {
    ...
    return {
        ...
        centroX: w * 0.40, // Centrado horizontal
        centroY: h * 0.40, // Centrado vertical
        spreadX: min * RAMO_FIJO.spreadX * 1.10,
        spreadY: min * RAMO_FIJO.spreadY * 1.10,
        baseX: w * 0.40,
        baseY: h * 0.70, // Base del ramo (relativo al canvas)
    };
}
...
B1.addEventListener('click', onReceive, {
    passive: true
});
